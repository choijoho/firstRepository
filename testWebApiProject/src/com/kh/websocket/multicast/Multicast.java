...
// configurator: 서버 설정에 관환 부분으로 서버 개통시에 기본적으로 해야할 설정 정보들을 담는 객체
@ServerEndpoint(value="/multicast/{chatRoom}", configurator=GetHttpSessionConfigurator.class)
public class Multicast {
	
	// 각 방 별로 사용자들의 정보를 담기 위한 Map 객체를 선언하여 각 방을 key로 사용자 리스트를 Set<Session> 컬렉션 객체로
   ...
   // 연결하려는 사용자의 정보와 접속한 방의 이름을 받아서 해당 방의 사용자 리스트에 갱신을 해주어야 한다.
    ...
    public void onMessage(String message, Session session) throws IOException {
    	...
    	// 현재는 사용하지 않지만 이후 확장가능성 즉, 나중에 사용자리스트를 표현하거나 어떤 사용자가 보낸
    	// 것인지 표현할 떄 사용할 수 있으므로 코드를 작성해 놓은 부분
      ...
      userList.stream().forEach( x -> {
    	  // 람다식 사용부:
    	  // 기존 절차지향 언어가 가지는 특성인 익명 함수 구현부분을 객체 지향언어에서도 흉내내기 위한 소스코드 작성 기술
    	  // x: Session
